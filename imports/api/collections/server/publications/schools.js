...
Meteor.publish("school_list", function() {
	return Schools.publishJoinedCursors(Schools.find({ownerId:this.userId}, {sort:{school:1}}));
});

Meteor.publish("schools_empty", function() {
	return Schools.publishJoinedCursors(Schools.find({_id:null,ownerId:this.userId}, {}));
});

Meteor.publish("school_details", function(schoolId) {
	return Schools.publishJoinedCursors(Schools.find({_id:schoolId,ownerId:this.userId}, {}));
});

Meteor.publish("school_list_paged", function(extraOptions) {
	extraOptions.doSkip = true;
	return Schools.publishJoinedCursors(Schools.find(databaseUtils.extendFilter({ownerId:this.userId}, extraOptions), databaseUtils.extendOptions({sort:{school:1}}, extraOptions)));
});
